...
# lire un item,
# vérifier si un chiffre existe dans item
# si oui, compteur+1
def count_number_journaliste(data_journaliste)
    count_num=0
...
# lire un item,
# vérifier si la chaine contient une maj
# si oui, compteur+1
def count_str_uppercase(data_journaliste)
    count_num=0
    data_journaliste.each do |handle|
        if handle.gsub(/[^A-Z]/, '') != ""
            count_num +=1
